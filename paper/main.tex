...
ratio holds at larger scale (790M+, 3060-tier hardware); (4) the gate
decision (protocol \S4.4, G1) this paper's findings bear on but do not
themselves make: Phases 2-5 as specified are machinery for using a
vocab-anchored workspace channel in the genuine state, and this paper's
results (thin/saturating legibility, non-localization, mostly-form
transplant decomposition) are evidence that channel isn't organized that
way -- state plainly whether that means Phases 2-5 are superseded by
this finding rather than merely unrun, since every downstream
``not yet done'' in this project inherits its status from that one call.}
